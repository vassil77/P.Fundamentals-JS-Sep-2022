function f2(params) {
    ...
    for (let line of params) {
        ...
        if (line.search(" posts on ") >= 0) {
            ...
            if (listUsers.includes(userName) && articleList.includes(article)) {
                ...
                commentList[article][titleContent] = userName;
    ...
    for (let article in commentList) {
        let sortedUsers = Object.entries(commentList[article]).sort((a, b) => a[1].localeCompare(b[1]));
    ...
    for (const article in commentList) {
        ...
        for (const el in commentList[article]) {
            // console.log(el);
            console.log(`--- From user ${commentList[article][el]}: ${el}`);
